...
export default async function PromosPage() {
  const products = await prisma.product.findMany({ where: { comparePriceCents: { not: null } }, include: { brand: true, images: true, reviews: true } });
  return (
    <>
      <header className="border-b border-white/5 py-16">
        <div className="container-luxe"><p className="text-xs uppercase tracking-[0.3em] text-gold-300">Sélection en offre</p><h1 className="mt-2 font-display text-display-xl">Promotions.</h1></div>
      </header>
