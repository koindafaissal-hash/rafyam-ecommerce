...
export default async function BestsellersPage() {
  const products = await prisma.product.findMany({ where: { isBestseller: true }, include: { brand: true, images: true, reviews: true } });
  return (
    <>
      <header className="border-b border-white/5 py-16">
        <div className="container-luxe"><p className="text-xs uppercase tracking-[0.3em] text-gold-300">Les préférées de nos clients</p><h1 className="mt-2 font-display text-display-xl">Meilleures <em className="text-gold-gradient not-italic">ventes.</em></h1></div>
      </header>
